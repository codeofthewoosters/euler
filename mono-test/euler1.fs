...
let rec findNextPrimeUnderNth (primesToX:int64 list) (x:int64) = 
    ...
    | head :: tail when List.length primesToX < 10001
        -> 
            ...
            | true 
                -> findNextPrime (x::primesToX) (x + 2L)
            | false 
                -> findNextPrime primesToX (x + 2L)
    | _ 
        -> primesToX |> Seq.max
...
let prods = 
    ...
    |> Seq.fold (fun acc i -> 
                let l = fst i
                let u = snd i
                let prod = digits.[l..u] |> Seq.reduce (*)
                prod :: acc
        ) [];;
...
let processH grd n = 
    ...
    let maxI seq adj = 
        Seq.windowed adj seq |> Seq.map (Array.reduce (*)) |> Seq.max
...
let processD grd n rv = 
    ...
    let prods = groups |> List.map (fun g -> List.reduce (*) g) 
